...
export  namespace Interpreter {
   ...
   export enum ActionStatus { ContinueAction, SucessAction, FailAction, StopAction }
    ...
    {
        query(x: any, y: any, fn: any): boolean {
    ...
    export class RelationOneMany<T extends Model.Kind, V extends Model.Kind> extends RelationDecl {
        ...
        query(x: T, y: V, fn: (a, b) => boolean): boolean {
            let ret = false;
            for (var [i, ab] of this.relations.entries()) {
                if (fn(ab[0], x)) {
                    for (var [j, b] of ab[1].entries()) {
   ...
   export  class RunTime {
       ...
       public isMatchItem(a: string | Model.Kind, b: string | Model.Kind): AssertReturn | Model.Kind
       {
           if (a == b) return AssertReturn.Equals;
           ...
           if (b instanceof Model.Kind) {
               if (typeof a == "string") {
                   if (b.inheritance.indexOf(a) >= 0) return b;
               }
           }
       ...
       public isMatch(h: DefinitionHeader, x, v, y)  {
           ...
           if (rx === AssertReturn.Undefined) return null
           return [rx,v,y]
       }
       ...
       public is_enum(arg0: Model.Kind, x: string): AssertReturn {
           for (var [i, v] of arg0.enumProperies.entries()) {
               if (v.values.indexOf(x) > -1) {
                   if (v.actualValue === x) return AssertReturn.Equals
       ...
       public is_k(arg0: Model.Kind | string , x: Model.Kind | string): boolean
       {
           let rmm = this.isMatchItem(arg0, x);
           if (rmm == AssertReturn.Equals) return true;
           if (rmm == AssertReturn.NotEquals) return false;
           
           if ((arg0 instanceof Model.Kind) && (typeof x == "string"))
           {
               let rr = this.is_enum(arg0, x)
               if (rr == AssertReturn.Equals) return true;
               if (rr == AssertReturn.NotEquals) return false;
           }
       ...
       public is(arg0: Model.Kind | string, obj_name: Model.Kind | string): boolean {

           let ax: Model.Kind | string = arg0
           let ox: Model.Kind | string = obj_name
           ...
           if (typeof obj_name == "string") {
               ...
               if (ox === null) ox = obj_name
           }
           if (typeof ax == "string") return this.is_k(ox, ax)
